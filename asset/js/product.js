// 1. Tải TẤT CẢ sản phẩm từ localStorage vào một biến tạm
let allProductsFromStorage = localStorage.getItem("listProducts")
...
// 2. !!! DÒNG SỬA Ở ĐÂY !!!
// Tạo ra mảng listProducts MỚI chỉ chứa các sản phẩm KHÔNG BỊ ẨN
let listProducts = allProductsFromStorage.filter(function(product) {
    return !product.isHidden; // Chỉ giữ lại sản phẩm có isHidden = false
});
...
function applyFilters() {
  ...
  if (typeVal !== "all") {
    filtered = filtered.filter((p) => p.nature.type === typeVal);
  }
...
function addToCart(productId) {
    ...
    if (userIndex !== -1) {
        ...
        if (productToAdd) {
            // ← Sửa: Chỉ gộp nếu cùng idProduct VÀ check == 0 (được chọn)
            // Sửa: tìm theo idProduct (bỏ điều kiện check). 
            // Nếu cần phân biệt biến thể (size/type) thì thêm so sánh tương ứng ở đây.
            const existingCartItemIndex = dataUsers[userIndex].cartItems.findIndex(
                (item) => item.idProduct == productId
            );

            if (existingCartItemIndex !== -1) {
                // Sản phẩm đã tồn tại và được chọn → tăng số lượng
                ...
                let cartItem = {
                    ...
                    check: 1, // default = not selected / unchecked
